<template>
    <div>
        <v-card class="pb-5" tile outlined>
            <v-card-title>
                <v-icon left>mdi-newspaper-plus</v-icon>Edit article
                ...
                <v-container class="pa-1">
                    <v-row class="mb-7 mx-5 mt-1">
                        <v-col cols="12" class="mb-12 text-center">
                            ...
                            <form enctype="multipart/form-data" ref="edit_image_form">
                                <picture-input
                                    :prefill="`/storage/images/${edit_resource.image_path}`"
                                    ref="pictureInput"
                                    margin="16"
                                    width="600"
                                    height="400"
        ...
        drag: 'Drag here or click to upload article cover photo'
                        ...
                        <v-col cols="3">
                            <v-autocomplete
                                ...
                                item-text="name"
                                item-value="id"
                                v-model="edit_resource.category_id"
...
export default {
    ...
    methods: {
        set_body(text) {
            this.edit_resource.body = text;
        },

        submit_data() {
            // console.log(this.edit_resource.tags);
            // return;
            let currentObj = this;
            let formdata = new FormData(this.$refs["edit_image_form"]);
            formdata.append("_method", "PUT");
            formdata.append("body", currentObj.edit_resource.body);
            formdata.append("title", currentObj.edit_resource.title);
            formdata.append("summary", currentObj.edit_resource.summary);
            currentObj.edit_resource.tags.forEach(tag => {
                formdata.append("tags[]", tag);
            });

            formdata.append(
                "category_id",
                currentObj.edit_resource.category_id
            );
